...
SELECT location, date, population, total_cases, (CAST(total_cases AS FLOAT) / CAST(population AS FLOAT))*100 AS perfcentPopulationInfected 
FROM PortfolioProject..covid_deaths
...
SELECT location, population, MAX(total_cases) AS highest_infection_count, MAX(CAST(total_cases AS FLOAT) / CAST(population AS FLOAT))*100 AS perfcentPopulationInfected 
FROM PortfolioProject..covid_deaths
-- WHERE location like '%states'
WHERE continent IS NOT NULL
...
-- Currently contains 'Income Levels High, Upper, and Low as 'location' metrics'.
...
SELECT  SUM(new_cases) AS total_cases, SUM(new_deaths) AS total_deaths, (SUM(CAST(new_deaths AS FLOAT))/SUM(CAST(new_cases AS FLOAT)))*100 AS globalDeathPercentage
FROM PortfolioProject..covid_deaths
